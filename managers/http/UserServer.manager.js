...
module.exports = class UserServer {
    ...
    _buildPayloadFromRoute({ route, req }){
        ...
        this._mergePayloadBucket(
            payload,
            'body',
            this._pickMappedValues({ source: req.params || {}, mapping: route.bodyFromParams }),
        );
        this._mergePayloadBucket(
            payload,
            'query',
            this._pickMappedValues({ source: req.params || {}, mapping: route.queryFromParams }),
        );
        this._mergePayloadBucket(
            payload,
            'body',
            this._pickMappedValues({ source: req.query || {}, mapping: route.bodyFromQuery }),
        );
        this._mergePayloadBucket(
            payload,
            'query',
            this._pickMappedValues({ source: req.body || {}, mapping: route.queryFromBody }),
        );
    ...
    _normalizeRestRoute({ moduleName, route, index }){
        if (!route || typeof route !== 'object') {
            throw Error(`invalid restExposed route for module ${moduleName} at index ${index}`);
        }

        const method = String(route.method || '').toLowerCase().trim();
        const path = route.path;
        const fnName = route.fnName;

        if (!method || typeof app[method] !== 'function') {
            throw Error(`invalid REST method on ${moduleName}.${fnName || 'unknown'} route at index ${index}`);
        }

        if (typeof path !== 'string' || !path.startsWith('/')) {
            throw Error(`invalid REST path on ${moduleName}.${fnName || 'unknown'} route at index ${index}`);
        }
    ...
    _buildRestRouteRegistry(){
        ...
        routes.forEach((route) => {
            ...
            if (seen.has(key)) {
                throw Error(`duplicate REST route definition: ${route.method.toUpperCase()} ${route.path}`);
            }
            seen.add(key);
        });
    ...
    _bindRestRoutes(){
        ...
        routes.forEach((route) => {
            app[route.method](route.path, call({
